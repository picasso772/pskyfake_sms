...
import com.psky.fake_sms.adapter.MessengerAdapter
...
class ShowListMessengerView : BaseFragment(), ItemClick {
    ...
    private val handler = object :Handler(){
        override fun handleMessage(msg: Message?) {
            when (msg?.what) {
                1 ->  {
                    ScreenService.shared.setSubView(MessengerChatViewController::class)
                }
            }
        }
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = MessengerAdapter(DataUtils.shared.users?.toMutableList() ?: mutableListOf(), context = context)
