...
import com.psky.fake_sms.adapter.LineAdapter
...
import kotlinx.android.synthetic.main.show_list_line_view.*

class ShowListLineView : BaseFragment(), ItemClick {
    ...
    private val handler = object : Handler(){
        override fun handleMessage(msg: Message?) {
            when (msg?.what) {
                1 ->  {
                    ScreenService.shared.setSubView(LineChatViewController::class)
                }
            }
        }
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = LineAdapter(DataUtils.shared.users?.toMutableList() ?: mutableListOf(), context = context)
