...
import kotlinx.android.synthetic.main.message_chat_friend.view.messageFriend
import kotlinx.android.synthetic.main.message_chat_friend_messenger.view.*
import kotlinx.android.synthetic.main.message_chat_me.view.*
import java.io.File

class MessengerChatAdapter(var items: MutableList<MessageChat>, val context: Context?) :
    ...
    override fun onCreateViewHolder(p0: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return when (viewType) {
            TYPE_HEADER -> {
                ...
                MessengerChatAdapter.HeaderViewHolder(view)
            }
            TYPE_ITEM_ME -> {
                view = LayoutInflater.from(context).inflate(R.layout.message_chat_me, p0, false)
                MessengerChatAdapter.MessageMeViewHolder(view)
            }
            TYPE_ITEM_FRIEND -> {
                view = LayoutInflater.from(context).inflate(R.layout.message_chat_friend_messenger, p0, false)
                MessengerChatAdapter.MessageFriendViewHolder(view)
            }
            else -> {
                view = LayoutInflater.from(context).inflate(R.layout.header_message_chat, p0, false)
                MessengerChatAdapter.HeaderViewHolder(view)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (holder) {
            is MessengerChatAdapter.HeaderViewHolder -> holder.title.text = items[position].content
            is MessengerChatAdapter.MessageFriendViewHolder -> {
            ...
            is MessengerChatAdapter.MessageMeViewHolder -> holder.messageMe.text = items[position].content
        }
